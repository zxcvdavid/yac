...
#ifndef CREATE_FILE_FAILED
#define CREATE_FILE_FAILED NULL
#endif
...
static int create_segments(unsigned long k_size, unsigned long v_size, yac_shared_segment_create_file **shared_segments_p, int *shared_segments_count, char **error_in) /* {{{ */ {
	...
	memfile = CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, allocate_size, NULL);
	first_segment.common.p = MapViewOfFile(memfile, FILE_MAP_ALL_ACCESS, 0, 0, 0);

	if (first_segment.common.p == CREATE_FILE_FAILED) {
		*error_in = "create_file";
		return 0;
	}
	...
	*shared_segments_p = (yac_shared_segment_create_file *)calloc(1, segments_num * sizeof(yac_shared_segment_create_file));
	if (!*shared_segments_p) {
		...
		UnmapViewOfFile(first_segment.common.p)*/;
	...
	*shared_segments_count = segments_num;
	...
	for (i = 1; i < segments_num; i++) {
		...
		(*shared_segments_p)[i].common.p = (void *)((unsigned long)first_segment.common.p + occupied_size);
...
static int detach_segment(yac_shared_segment *shared_segment) /* {{{ */ {
	if (shared_segment->p) {
		UnmapViewOfFile(shared_segment->p);
		CloseHandle(memfile);
	}
	return 0;
}
